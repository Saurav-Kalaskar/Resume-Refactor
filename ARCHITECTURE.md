...
| **3** | **Backend – `main.py`** | Calls **`extract_keywords`** (fast model) from `keywords.py`. | `extract_keywords` returns a dict with: `all_keywords`, `company_name`, `company_mission_and_product`, `core_problems_to_solve`. Model: `settings.FAST_MODEL` (**openai/gpt‑oss‑20b**). |
| **4** | **Backend – `main.py`** | Calls **`generate_bullets`** from `llm.py` using `request.model` or default **REASONING_MODEL** (**qwen/qwen3‑next‑80b‑a3b‑instruct**). | LLM receives JD, original LaTeX sections (Professional Experience & Projects), `company_mission`, and `core_problems`. Returns JSON object with updated bullet lists. |
| **5** | **Backend – `main.py`** | **`bold_keywords_in_bullets`** normalises each section and wraps each extracted keyword (`max_keywords = 15`) in `\textbf{}`. | Function lives in `main.py`, not `keywords.py`. |
...
| **9** | **Backend – `main.py`** | Assembles `RefactorResponse` – includes `status`, `message`, `pdf_base64`, `latex_source`, `bullets_applied`, `keywords_found[:15]`, **and the exact `company_name`** from step 3. | No fallback for missing `company_name`. |
...
|---------|----------------|---------------|-------------------|
| **Fast extraction** (company name + keywords + strategic context) | **openai/gpt‑oss‑20b** (`FAST_MODEL`) | `backend/app/keywords.py → extract_keywords` | System prompt asks for **exact JSON object**: `{ "company_name": "...", "all_keywords": [...], "company_mission_and_product": "...", "core_problems_to_solve": "..." }`. No markdown, no extra text. |
| **Reasoning / bullet generation** | **qwen/qwen3‑next‑80b‑a3b‑instruct** (`REASONING_MODEL`) | `backend/app/llm.py → generate_bullets` | System prompt defines strict bullet schema; user prompt supplies JD, current LaTeX sections, `company_mission`, and `core_problems`. Output must be a JSON object matching the schema. |
...
* **Backend** orchestrates two distinct LLM calls:
  1. **Fast extraction** (FAST_MODEL) returns **company_name**, **all_keywords**, **company_mission_and_product**, and **core_problems_to_solve** in one JSON object.
  2. **Reasoning** (REASONING_MODEL) rewrites bullets according to a strict schema, using strategic context (mission + problems) from step 1.
* Keywords are bolded in `main.py::bold_keywords_in_bullets()`, bullets injected into LaTeX via `refactor_bridge.py`, PDF compiled with `pdflatex`, and a comprehensive response returned.
